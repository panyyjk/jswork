...
var storage = multer.diskKtorage({
...
app.listen(8080, () => console.log('node espress 服务器已启动，监听端口： 8080'))
...
var server = ws.createServer(function (conn){
    id++
    conn.name = "p"+id
    BroadcastChannel(server,'有新人加入.')
    conn.on("text",function (str){
        if(str.slice(0,9)=='nickname|'){
            conn.name=str.split('|')[1]
            BroadcastChannel(server,conn.name+'上线了。')
            return
        }
...
}).listen(8081,()=>console.log('sock server listening on:8081'))
